...
- Overloaded RSU queue saturation (TEST 5B)
- Normalized Eq. 23 task priority scale & alpha/beta sensitivity (TEST 6, TEST 7)
- Controlled deadline priority ordering sensitivity (TEST 7B)
- Action semantics and physical routing (TEST 8)
- Collaborative workload conservation phi_1 + phi_rest == phi_total (TEST 8B)
- Baseline fairness (CoTOP, Local, Greedy across identical workload) (TEST 9)
- Edge cases and lifecycle invariants (TEST 10)
- Task CPU-demand semantics sensitivity (fixed 10 Mcycles vs uniform) (TEST 11)
"""
import pytest
import math
import numpy as np
...
from utils.task_priority import compute_task_priority_paper, compute_task_priority_normalized, compute_task_priority, prioritize_tasks
...
# =========================================================================
# TEST 5B: Overloaded RSU Queue Saturation
# =========================================================================
def test_overloaded_rsu_queue_saturation():
    # RSU with 1 GHz capacity subjected to continuous 2 Gcycles/s workload
    rsu = RSU(0, (0.0, 0.0), cpu_capacity_f=1.0e9, queued_cpu_cycles=0.0, transmission_power_P_R=100.0)
    dt = 1.0
    arrival_rate = 2.0e9  # 2 Gcycles / s (> service rate 1 Gcycles / s)

    history = []
    for step in range(10):
        rsu.queued_cpu_cycles += arrival_rate
        rsu.queued_cpu_cycles = max(0.0, rsu.queued_cpu_cycles - rsu.cpu_capacity_f * dt)
        history.append(rsu.queued_cpu_cycles)

    # Queue must strictly monotonically increase under overload
    for i in range(1, len(history)):
        assert history[i] > history[i-1]
    assert pytest.approx(history[-1], rel=1e-5) == 10.0e9
...
def test_task_priority_scale_balance():
    ...
    p_min = compute_task_priority_normalized(t_min, dwell_time=0.5, alpha=0.3, beta=0.7)
    p_max = compute_task_priority_normalized(t_max, dwell_time=100.0, alpha=0.3, beta=0.7)
...
# =========================================================================
# TEST 7B: Controlled Deadline Priority Ordering Sensitivity
# =========================================================================
def test_deadline_priority_ordering_sensitivity():
    # Construct 3 tasks with identical data size and dwell time, varying deadline
    t_urgent = Task(1, "v1", size_rho=3.0e6, cpu_phi=10.0e6, max_delay_d=20.0)
    t_medium = Task(2, "v1", size_rho=3.0e6, cpu_phi=10.0e6, max_delay_d=25.0)
    t_relaxed = Task(3, "v1", size_rho=3.0e6, cpu_phi=10.0e6, max_delay_d=30.0)

    p_urgent = compute_task_priority(t_urgent, dwell_time=10.0, alpha=0.3, beta=0.7)
    p_medium = compute_task_priority(t_medium, dwell_time=10.0, alpha=0.3, beta=0.7)
    p_relaxed = compute_task_priority(t_relaxed, dwell_time=10.0, alpha=0.3, beta=0.7)

    # Shorter deadline must yield strictly higher priority score
    assert p_urgent > p_medium > p_relaxed
...
# =========================================================================
# TEST 8B: Collaborative Workload Conservation
# =========================================================================
def test_collaborative_workload_conservation():
    # Test across a sweep of task sizes, CPU demands, and RSU capacities
    for total_phi in [2.0e6, 5.0e6, 10.0e6]:
        for rsu1_f in [1.0e9, 2.0e9, 4.0e9]:
            for rsu2_f in [1.0e9, 2.0e9, 4.0e9]:
                for dwell in [0.001, 0.005, 0.010, 0.5]:
                    # Closed-form calculation of workload partition
                    t_comp1_full = total_phi / rsu1_f
                    if dwell < t_comp1_full:
                        t1 = max(dwell, 0.0)
                    else:
                        part_ratio = rsu1_f / (rsu1_f + rsu2_f)
                        t1 = min(dwell, part_ratio * t_comp1_full)

                    phi_1 = min(rsu1_f * t1, total_phi)
                    phi_rest = max(total_phi - phi_1, 0.0)

                    # Conservation law: phi_1 + phi_rest == total_phi
                    assert pytest.approx(phi_1 + phi_rest, rel=1e-9) == total_phi
                    assert 0.0 <= phi_1 <= total_phi
                    assert 0.0 <= phi_rest <= total_phi
...
# =========================================================================
# TEST 11: Task CPU-Demand Semantics Sensitivity
# =========================================================================
def test_task_cpu_demand_semantics_sensitivity(base_config):
    """
    Compares analytical delay under fixed 10 Mcycles vs Uniform(1, 10) Mcycles.
    Validates that:
    1. Fixed 10 Mcycles increases mean compute latency by ~1.8x over uniform mean (~5.5 Mcycles).
    2. Under Table III RSU capacities (1-4 GHz), pure compute latency is 0.0025-0.010 s,
       meaning this interpretation shift contributes < 0.005 s total latency variation
       and cannot explain the ~12.5 s numerical scale gap.
    """
    rsu_cpu_f = 2.0e9  # 2 GHz average
    fixed_phi = 10.0e6  # 10 Mcycles
    uniform_phi_mean = 5.5e6  # (1 + 10) / 2 = 5.5 Mcycles

    t_comp_fixed = fixed_phi / rsu_cpu_f
    t_comp_uniform = uniform_phi_mean / rsu_cpu_f

    delta_comp = t_comp_fixed - t_comp_uniform
    assert pytest.approx(t_comp_fixed, rel=1e-5) == 0.005  # 5 ms
    assert pytest.approx(t_comp_uniform, rel=1e-5) == 0.00275  # 2.75 ms
    assert delta_comp < 0.005, f"Compute latency difference is only {delta_comp}s, which is negligible"
