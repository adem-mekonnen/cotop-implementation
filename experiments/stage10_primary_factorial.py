...
def run_cell_task(task_args: Tuple) -> Dict[str, Any]:
    ...
    if not overwrite and os.path.exists(manifest_path) and os.path.exists(ckpt_path) and os.path.exists(metrics_path) and os.path.exists(eval_results_path):
        try:
            ...
            with open(metrics_path, "r") as f:
                metrics = json.load(f)
            print(f"[CACHE HIT] Cell {algo} | {geom} | w{workload} | Seed {seed} already complete. Skipping.")
    ...
    print(f"[SUCCESS] Cell {cell_summary['cell_id']} complete! Delay: {cell_summary['mean_delay_s']}s, Energy: {cell_summary['mean_energy_j']}J, Comp: {cell_summary['completion_ratio']*100:.1f}%")
    return cell_summary
...
def main():
    ...
    print("=" * 80)
    print(f"   STAGE 10: RUNNING FULL PRIMARY FACTORIAL EXPERIMENT MATRIX")
    print(f"   Total Cells: {len(tasks)} (2 Geometries x 2 Algorithms x 3 Workloads x 5 Seeds)")
    ...
    print("\n" + "=" * 80)
    print(f"[COMPLETE] Full Primary Factorial Matrix ({len(summary_records)}/60 cells) executed successfully.")
