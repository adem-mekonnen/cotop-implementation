...
# Define the folders to ensure they exist
for d in ['envs', 'models', 'utils', 'configs', 'results/checkpoints', 'sumo_config']:
    os.makedirs(d, exist_ok=True)

# 1. Synchronized Entities (Eq. 25 & Table III)
...
# 2. Corrected Task Generator (Fixed Units & Table III sampling)
...
class TaskGenerator:
    ...
    def generate_tasks_for_vehicle(self, v_id):
        return [Task(i, v_id, random.uniform(*self.config.task_size_range), random.uniform(1e6, self.config.max_task_cpu*1e6), random.uniform(*self.config.task_deadline_range)) for i in range(self.config.num_tasks_per_vehicle_range[0])]
""")

# 3. Corrected Task Priority (Eq. 23)
with open('utils/task_priority.py', 'w') as f:
    f.write("""import math
def prioritize_tasks(tasks, t_stay, alpha=0.3, beta=0.7):
    for t in tasks:
        term1 = math.exp(-1.0 / t_stay) if t_stay > 0 else 0
        term2 = t.size_rho / (t.max_delay_d * 1e6) # Normalized scale
        t.priority = (alpha * term1) + (beta * term2)
    return sorted(tasks, key=lambda x: x.priority, reverse=True)
""")

# 4. Corrected VEC Environment (Mobility Wired In + Nearest RSU Baseline Fix)
...
from models.mobility_gat import MobilityGAT_GRU; from utils.task_priority import prioritize_tasks
class VECEnv(gym.Env):
    ...
    def step(self, action):
        ...
        return self._get_obs(), float(reward), done, False, {"delay": standalone_delay, "energy": energy, "missed_deadline": standalone_delay > task.max_delay_d}
...
print("✅ REPAIR COMPLETE. All files overwritten with clean, synchronized logic.")